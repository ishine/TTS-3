...
class Vits(BaseTTS):
    ...
    def on_train_step_start(self, trainer):
        ...
        if self.config.binary_alignment_loss_warmup_epochs > 0:
            self.binary_loss_weight = min(trainer.epochs_done / self.config.binary_alignment_loss_warmup_epochs, 1.0) * 1.0
        else:
            self.binary_loss_weight = 1.0
        self.binarize_alignment = trainer.total_steps_done >  self.config.binarization_start_steps

    def xon_init_end(self, trainer):  # pylint: disable=W0613
    ...
    def forward_duration_predictor(self, outputs, attn, x, x_mask, g, lang_emb):
        # duration predictor
    ...
    def _forward_aligner(
        ...
        aligner_soft = aligner_soft.transpose(2, 3)  # [B, 1, T_max2, T_max] -> [B, 1, T_max, T_max2]
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        outputs = self.forward_duration_predictor(outputs, aligner_hard, o_en, x_mask, g, lang_emb)
