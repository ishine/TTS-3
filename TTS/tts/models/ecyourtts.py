...
from TTS.tts.layers.ecyourtts.denoise import VitsDenoiser
from TTS.tts.layers.ecyourtts.discriminator import VitsDiscriminator
from TTS.tts.layers.ecyourtts.learned_upsampling import GaussianUpsampling, RangePredictor
from TTS.tts.layers.ecyourtts.networks import ContextEncoder, PosteriorEncoder, ResidualCouplingBlocks, TextEncoder
...
#############################
@dataclass
class EcyourTTSAudioConfig(Coqpit):
    fft_size: int = 1024
...
class EcyourTTSF0Dataset(F0Dataset):
...
class EcyourTTSDataset(TTSDataset):
    def __init__(self, *args, **kwargs):
        ...
        if self.compute_f0:
            self.f0_dataset = EcyourTTSF0Dataset(
...
@dataclass
class EcyourTTSArgs(Coqpit):
    """ECYourTTS model arguments.
...
class EcyourTTS(BaseTTS):
    ...
    Check :class:`TTS.tts.configs.ecyourtts_config.EcyourTTSConfig` for class arguments.

    Examples:
        >>> from TTS.tts.configs.ecyourtts_config import EcyourTTSConfig
        >>> from TTS.tts.models.ecyourtts import EcyourTTS
        >>> config = EcyourTTSConfig()
        >>> model = EcyourTTS(config)
    """
    ...
    def get_criterion(self):
        ...
        if self.config.train_dp:
            ...
            return [EcyourTTSDiscriminatorLoss(self.config), EcyourTTSGeneratorLoss(self.config)]
    ...
    @staticmethod
    def init_from_config(config: "EcyourTTSConfig", samples: Union[List[List], List[Dict]] = None, verbose=True):
        """Initiate model from config

        Args:
            config (EcyourTTSConfig): Model config.
        ...
        return EcyourTTS(new_config, ap, tokenizer, speaker_manager, language_manager, emotion_manager)
...
class EcyourTTSGeneratorLoss(nn.Module):
...
class EcyourTTSDiscriminatorLoss(nn.Module):
    def __init__(self, c: Coqpit):
        super().__init__()
...
##################################
# ECYourTTS CHARACTERS
##################################


class EcyourTTSCharacters(BaseCharacters):
    """Characters class for ECYourTTS model for compatibility with pre-trained models"""
...
@dataclass
class EcyourTTSConfig(BaseTTSConfig):
    """Defines parameters for ECYourTTS End2End TTS model.

    Args:
        model (str):
            Model name. Do not change unless you know what you are doing.

        model_args (EcyourTTSArgs):
            Model architecture arguments. Defaults to `EcyourTTSArgs()`.

        audio (EcyourTTSAudioConfig):
            Audio processing configuration. Defaults to `EcyourTTSAudioConfig()`.
    ...
    model_args: EcyourTTSArgs = field(default_factory=EcyourTTSArgs)
    audio: EcyourTTSAudioConfig = EcyourTTSAudioConfig()
...
if __name__ == "__main__":
    ...
    model = EcyourTTS.init_from_config(config)
    model.cuda()
