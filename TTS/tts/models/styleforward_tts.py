...
class StyleforwardTTS(BaseTTS):
    ...
    def forward(
        ...
        if(self.config.style_encoder_config.use_cycle_consistency):
            ...
            if(self.config.style_encoder_config.use_lookup):
                ...
                o_en_cycle = encoder_outputs_cycle.permute(0,2,1)
                style_encoder_outputs_cycle = self.style_encoder_layer.forward(inputs = o_en_cycle, style_mel=y , mel_mask = y_lengths)
            ...
            # decoder pass
            print(f'oen cycle = {o_en_cycle}')
            print(f'Lengths only cycle = {y_lengths_cycle}')
    ...
    def inference(self, x, aux_input={"d_vectors": None, "speaker_ids": None, "cond_speaker_ids" : None, 'style_mel': None, "style_ids": None,'pitch_control': None, 'pitch_replace': None, 'z': None}):  # pylint: disable=unused-argument
        ...
        if(self.config.style_encoder_config.use_lookup):
            ...
            se_inputs = [o_en.permute(0,2,1), aux_input['style_mel']]
            style_encoder_outputs = self.style_encoder_layer.forward(inputs = o_en.permute(0,2,1), style_mel=aux_input['style_mel'] , speaker_embedding = g_emb.permute(1,0).unsqueeze(1))
            ...
            # se_inputs = [o_en.permute(0,2,1), aux_input['style_mel']]
            style_encoder_outputs = self.style_encoder_layer.inference(o_en.permute(0,2,1), style_mel = aux_input['style_mel'], z = aux_input['z'])
            o_en = style_encoder_outputs['styled_inputs'].permute(0,2,1)
        elif(self.config.style_encoder_config.se_type == 'metastyle'):
            ...
            # se_inputs = [o_en.permute(0,2,1), aux_input['style_mel']]
            style_encoder_outputs = self.style_encoder_layer.inference(o_en.permute(0,2,1), style_mel = aux_input['style_mel'])
            o_en = style_encoder_outputs['styled_inputs'].permute(0,2,1)
