...
class StyleforwardTTS(BaseTTS):
    ...
    def inference(self, x, aux_input={"d_vectors": None, "speaker_ids": None, "cond_speaker_ids" : None, 'style_mel': None, "style_ids": None,'pitch_control': None, 'pitch_replace': None, 'z': None}):  # pylint: disable=unused-argument
        ...
        if(self.config.style_encoder_config.use_lookup):
            ...
            se_inputs = [o_en.permute(0,2,1), aux_input['style_mel']]
            style_encoder_outputs = self.style_encoder_layer.forward(inputs = o_en.permute(0,2,1), style_mel=aux_input['style_mel'] , speaker_embedding = g_emb.unsqueeze(1))
        ...
        if(self.config.style_encoder_config.use_clip_loss or self.config.style_encoder_config.use_style_distortion_loss):
            if(self.config.style_encoder_config.se_type=='modifiedre'):
                ...
                ressynt_style_encoder_output = self.style_encoder_layer.forward(inputs = o_en.permute(0,2,1), style_mel=o_de , speaker_embedding = g_emb.unsqueeze(1))['style_embedding']
            else:
