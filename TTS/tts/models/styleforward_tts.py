...
class StyleforwardTTS(BaseTTS):
    ...
    def inference(self, x, aux_input={"d_vectors": None, "speaker_ids": None, "cond_speaker_ids" : None, 'style_mel': None, "style_ids": None,'pitch_control': None, 'pitch_replace': None}):  # pylint: disable=unused-argument
        ...
        if(self.config.style_encoder_config.use_lookup):
            ...
            style_encoder_outputs = self.style_encoder_layer.forward(se_inputs, text_len = x_lengths, mel_len = style_mel_lengths)
            o_en = style_encoder_outputs['styled_inputs'].permute(0,2,1)
        elif(self.config.style_encoder_config.se_type == 'vae'):
            # se_inputs = [o_en.permute(0,2,1), aux_input['style_mel']]
            style_encoder_outputs = self.style_encoder_layer.forward(o_en.permute(0,2,1), z = aux_input['style_mel'])
            o_en = style_encoder_outputs['styled_inputs'].permute(0,2,1)
        else:
            se_inputs = [o_en.permute(0,2,1), aux_input['style_mel']]
            style_encoder_outputs = self.style_encoder_layer.forward(se_inputs)
