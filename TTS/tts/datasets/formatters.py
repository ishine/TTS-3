...
def vctk(root_path, meta_files=None, wavs_path="wav48_silence_trimmed", mic="mic1", ignored_speakers=None):
    ...
    for meta_file in meta_files:
        ...
        if os.path.exists(wav_file):
            items.append(
                [text, wav_file, "VCTK_" + speaker_id, style_name]
            )
        else:
...
def vctk_old(root_path, meta_files=None, wavs_path="wav48", ignored_speakers=None):
    ...
    for meta_file in meta_files:
        ...
        items.append(
                [text, wav_file, "VCTK_old" + speaker_id, style_name]
            )
    return items
...
def cpqd_style_read(root_path, meta_file, **kwargs):
    '''
        Generic style reader for files with multi-speaker
...
def emolj_style_read(root_path, meta_file, **kwargs):
    '''
        Generic style reader for files with multi-speaker
...
def emovdb(root_path, meta_file, **kwargs):
    '''
...
def iemocap(root_path, meta_file, **kwargs):
    '''
        Generic style reader for files with multi-speaker multi-style
        Input file must be a textual file separated by "|" with "\n" breaklines,
        and the columns must be:
