...
class CharVocos(Vocos):
    @classmethod
    def from_local_pretrained(cls, config_path: str, model_path: str) -> Vocos:
        """
        Same as above, except it takes local config files and parameter dictionaries.
        """
        model = cls.from_hparams(config_path)
        state_dict = torch.load(model_path, map_location="cpu")['state_dict']
        state_dict = {k: state_dict[k] for k in state_dict.keys() if not any(forbidden_key in k for forbidden_key in ['multiresddisc', 'multiperioddisc', 'melspec_loss', 'asr_model'])}
        ...
        print(
            f'Loaded CharVocos (encodec) from checkpoint in path {model_path}.\n\tMissing keys: {missing_keys}\n'
